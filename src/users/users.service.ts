...
export class UsersService {
  ...
  async reg(user:CreateUserDto){
    ...
    const user_res = await this.usersRepository.findOne({account: user.account});
    ...
    try {
      ...
      /*
        throw之后、后边的return还会不会执行?
        若不执行的话、catch中的code和msg则没用了
      */
    }
    return {code, msg}
  }
  ...
  // 
  // asyn ttts(account : string){
  //   return await this.usersRepository.findOne({account: account});
  // };
